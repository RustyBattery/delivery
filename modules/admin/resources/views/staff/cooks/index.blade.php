    ...
    <div class="py-12">
        <div class="p-2 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @foreach($cooks as $cook)
                <a href="{{route('user.show', [$cook->id])}}" class="block p-4 sm:p-8 bg-white shadow sm:rounded-lg hover:bg-gray-50 transition delay-75">
                    <p class="text-lg font-semibold">{{$cook->name}}</p>
                    <p class="mt-2">{{$cook->email}}</p>
                    <p class="mt-2">{{$cook->phone}}</p>
                </a>
            @endforeach
        </div>
    </div>
